...
import static com.sms.courier.engine.grpc.loadbalancer.Constants.ENGINE_NAME;
...
/**
 * A {@link LoadBalancer} that provides round-robin load-balancing over the {@link EquivalentAddressGroup}s from the
 * {@link NameResolver}.
 */
@Slf4j
final class CustomRoundRobinLoadBalancer extends LoadBalancer {
    ...
    private void processSubchannelState(Subchannel subchannel, ConnectivityStateInfo stateInfo) {
        ...
        String name = subchannel.getAttributes().get(ENGINE_NAME);
        applicationContext
            .publishEvent(new EngineStatusEvent(name, EngineStatus.getByConnectivityState(stateInfo.getState())));
        updateBalancingState();
    }
    ...
    private void updateBalancingState() {
        ...
        if (activeList.isEmpty()) {
            ...
            updateBalancingState(READY, new ReadyPicker(activeList, startIndex, applicationContext));
        }
    }
    ...
    /**
     * Converts list of {@link EquivalentAddressGroup} to {@link EquivalentAddressGroup} set and remove all attributes.
     * The values are the original EAGs.
     */
    ...
    static final class ReadyPicker extends RoundRobinPicker {
        ...
        ReadyPicker(List<Subchannel> list, int startIndex, ApplicationContext applicationContext) {
        ...
        public PickResult pickSubchannel(PickSubchannelArgs args) {
            ...
            String taskType = args.getCallOptions().getOption(TASK_TYPE);
            Attributes attributes = subchannel.getAttributes();
            String host = attributes.get(ENGINE_NAME);
            String jobId = args.getHeaders().get(JOB_ID);
            String jobType = args.getHeaders().get(JOT_TYPE);
            applicationContext.publishEvent(new EngineTaskRecordEvent(jobId, host, taskType, jobType));
