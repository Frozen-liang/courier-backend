...
import com.sms.courier.engine.grpc.api.v1.*;
import com.sms.courier.entity.api.common.*;
...
import com.sms.courier.entity.job.common.*;
...
public class GrpcMapperTest {
    ...
    private final JobEnvironment jobEnv = JobEnvironment.builder()
            .id(ID).projectId(ID).envName(NAME).envDesc(ENVDESC).frontUri("frontUri").afterInject("afterInject").beforeInject(INJECT).globalAfterProcess(PROCESS).globalBeforeProcess(PROCESS).requestParams(List.of(ParamInfo.builder().key(KEY).build())).requestParamType(1).headers(List.of(ParamInfo.builder().key(KEY).build())).envVariable(List.of(ParamInfo.builder().key(KEY).build())).urlParams(List.of(ParamInfo.builder().key(KEY).build())).requestParams(List.of(ParamInfo.builder().key(KEY).build())).build();
    private final JobDataCollection jobDataCollection = JobDataCollection.builder()
            .id(ID).projectId(ID).collectionName(NAME).testData(TestData.builder().dataName(NAME).data(List.of(DataParam.builder().value(VALUE).key(KEY).build())).build()).build();
    ...
    public void toGrpcJobRequest_SceneCaseJobEntity() {
        SceneCaseJobEntity sceneCaseJobEntity = builder()
                .id(ID).createDateTime(DATA).modifyDateTime(DATA).createUserId(ID).modifyUserId(ID).engineId(ID).projectId(ID).workspaceId(ID).environment(jobEnv).dataCollection(jobDataCollection).createUserName(NAME).apiTestCase(List.of(JobSceneCaseApi.builder().caseId(ID).build())).build();
        ...
        assertThat(sceneCaseJobEntity.getEnvironment().getId()).isEqualTo(grpcSceneCaseJobRequest.getEnvironment().getId());
        assertThat(sceneCaseJobEntity.getEnvironment().getProjectId()).isEqualTo(grpcSceneCaseJobRequest.getEnvironment().getProjectId());
        assertThat(sceneCaseJobEntity.getEnvironment().getEnvDesc()).isEqualTo(grpcSceneCaseJobRequest.getEnvironment().getEnvDesc());
        assertThat(sceneCaseJobEntity.getEnvironment().getHeaders().get(0).getKey()).isEqualTo(grpcSceneCaseJobRequest.getEnvironment().getHeaders(0).getKey());
        assertThat(sceneCaseJobEntity.getDataCollection().getId()).isEqualTo(grpcSceneCaseJobRequest.getDataCollection().getId());
        assertThat(sceneCaseJobEntity.getDataCollection().getTestData().getData().get(0).getValue()).isEqualTo(grpcSceneCaseJobRequest.getDataCollection().getTestData().getData(0).getValue());
        assertThat(sceneCaseJobEntity.getDataCollection().getTestData().getData().get(0).getKey()).isEqualTo(grpcSceneCaseJobRequest.getDataCollection().getTestData().getData(0).getKey());
        assertThat(sceneCaseJobEntity.getCreateUserName()).isEqualTo(grpcSceneCaseJobRequest.getCreateUserName());
        assertThat(grpcSceneCaseJobRequest.getApiTestCase(0).getCaseId()).isEqualTo(sceneCaseJobEntity.getApiTestCase().get(0).getCaseId());
    ...
    public void toGrpcJobRequest_ScheduleSceneCaseJobEntity() {
        ScheduleSceneCaseJobEntity scheduleSceneCaseJobEntity = ScheduleSceneCaseJobEntity
                .builder().id(ID).createDateTime(DATA).modifyDateTime(DATA).engineId(ID).workspaceId(ID).projectId(ID).environment(jobEnv).dataCollection(jobDataCollection).apiTestCase(List.of(JobSceneCaseApi.builder().sceneCaseId(ID).build())).build();
        ...
        assertThat(grpcSceneCaseJobRequest.getEnvironment().getId()).isEqualTo(scheduleSceneCaseJobEntity.getEnvironment().getId());
        assertThat(grpcSceneCaseJobRequest.getDataCollection().getId()).isEqualTo(scheduleSceneCaseJobEntity.getDataCollection().getId());
        assertThat(grpcSceneCaseJobRequest.getApiTestCaseList().get(0).getSceneCaseId()).isEqualTo(scheduleSceneCaseJobEntity.getApiTestCase().get(0).getSceneCaseId());
    }

    @Test
    public void toGrpcJobRequest_ScheduleCaseJobEntity() {
        ScheduleCaseJobEntity scheduleCaseJobEntity = ScheduleCaseJobEntity
                .builder().id(ID).createDateTime(DATA).modifyDateTime(DATA).engineId(ID).workspaceId(ID).projectId(ID).environment(jobEnv).dataCollection(jobDataCollection).apiTestCase(jobCaseApi).build();
        ...
        assertThat(grpcCaseJobRequest.getEnvironment().getId()).isEqualTo(scheduleCaseJobEntity.getEnvironment().getId());
        assertThat(grpcCaseJobRequest.getDataCollection().getId()).isEqualTo(scheduleCaseJobEntity.getDataCollection().getId());
        assertThat(grpcCaseJobRequest.getApiTestCase().getJobApiTestCase().getId()).isEqualTo(scheduleCaseJobEntity.getApiTestCase().getJobApiTestCase().getId());
    }

    @Test
    public void toGrpcJobRequest_ApiTestCaseJobEntity() {
        ApiTestCaseJobEntity apiTestCaseJobEntity = ApiTestCaseJobEntity
                .builder().id(ID).createDateTime(DATA).modifyDateTime(DATA).createUserId(ID).modifyUserId(ID).engineId(ID).workspaceId(ID).projectId(ID).environment(jobEnv).dataCollection(jobDataCollection).apiTestCase(jobCaseApi).createUserName(NAME).build();
        ...
        assertThat(grpcCaseJobRequest.getEnvironment().getId()).isEqualTo(apiTestCaseJobEntity.getEnvironment().getId());
        assertThat(grpcCaseJobRequest.getDataCollection().getId()).isEqualTo(apiTestCaseJobEntity.getDataCollection().getId());
        assertThat(grpcCaseJobRequest.getApiTestCase().getJobApiTestCase().getId()).isEqualTo(apiTestCaseJobEntity.getApiTestCase().getJobApiTestCase().getId());
    ...
    public void GrpcParamInfo_toGrpcParamInfo() {
        ParamInfo paramInfo = ParamInfo.builder().required(Boolean.TRUE).key(KEY).value(VALUE).paramType(PARAM_TYPE).mockRule(MOCK_RULE).childParam(List.of(ParamInfo.builder().key(KEY).build())).build();
        ...
        assertThat(grpcParamInfo.getChildParamList().get(0).getKey()).isEqualTo(paramInfo.getChildParam().get(0).getKey());
    }

    @Test
    public void GrpcParamInfo_toGrpcMatchParamInfo() {
        MatchParamInfo matchParamInfo = MatchParamInfo.builder()
                .key(KEY).value(VALUE).matchType(MATCH_TYPE).paramType(PARAM_TYPE).childParam(List.of(MatchParamInfo.builder().key(KEY).build())).build();
        ...
        assertThat(grpcMatchParamInfo.getChildParam(0).getKey()).isEqualTo(matchParamInfo.getChildParam().get(0).getKey());
    }

    @Test
    public void GrpcJobSceneCaseApi_JobSceneCaseApi() {
        JobSceneCaseApi jobSceneCaseApi = JobSceneCaseApi.builder()
                .caseId(ID).id(ID).sceneCaseId(ID).caseTemplateId(ID).apiType(ApiType.getApiType(1)).shell(SHELL).order(ORDER).jobApiTestCase(JobApiTestCase.builder().id(ID).build()).build();
        ...
        assertThat(jobSceneCaseApi.getJobApiTestCase().getId()).isEqualTo(grpcJobSceneCaseApi.getJobApiTestCase().getId());
    }
    ...
    public void GrpcJobSceneCaseApi_JobApiTestCase() {
        ...
        JobApiTestCase jobApiTestCase = JobApiTestCase.builder()
                .id(ID).caseName(NAME).responseParamsExtractionType(ResponseParamsExtractionType.getType(1)).httpStatusVerification(httpStatusVerification).responseHeadersVerification(responseHeadersVerification).responseResultVerification(responseResultVerification).responseTimeVerification(responseTimeVerification).dataCollId(ID).advancedSetting(new AdvancedSetting(Duration.ofMillis(10), true)).jobApi(JobApi.builder().apiName(NAME).build()).build();
        ...
        assertThat(jobApiTestCase.getResponseParamsExtractionType().getCode()).isEqualTo(grpcJobApiTestCase.getResponseParamsExtractionType());
        assertThat(jobApiTestCase.getHttpStatusVerification().getStatusCode()).isEqualTo(grpcJobApiTestCase.getHttpStatusVerification().getStatusCode());
        assertThat(jobApiTestCase.getResponseHeadersVerification().getParams().get(0).getKey()).isEqualTo(grpcJobApiTestCase.getResponseHeadersVerification().getParams(0).getKey());
        assertThat(jobApiTestCase.getResponseResultVerification().getParams().get(0).getKey()).isEqualTo(grpcJobApiTestCase.getResponseResultVerification().getParams(0).getKey());
    ...
    public void GrpcJobSceneCaseApi_toGrpcJobApiResponse() {
        ...
        JobApi jobApi = JobApi.builder()
                            .projectId(ID).apiName(NAME).description(description).apiPath(apiPath).apiProtocol(ApiProtocol.getType(1)).requestMethod(RequestMethod.getType(1)).apiRequestParamType(ApiRequestParamType.getType(1)).requestRaw(responseRaw).requestRawType(RawType.getType(1)).preInject(INJECT).postInject(INJECT).apiRequestJsonType(ApiJsonType.getType(1)).apiResponseJsonType(ApiJsonType.getType(1)).apiResponseParamType(ApiRequestParamType.getType(1)).responseRaw(responseRaw).responseRawType(RawType.getType(1)).requestHeaders(List.of(ParamInfo.builder().key(KEY).build())).responseHeaders(List.of(ParamInfo.builder().key(KEY).build())).pathParams(List.of(ParamInfo.builder().key(KEY).build())).restfulParams(List.of(ParamInfo.builder().key(KEY).build())).requestParams(List.of(ParamInfo.builder().key(KEY).build())).responseParams(List.of(ParamInfo.builder().key(KEY).build())).build();
    ...
    public void GrpcResponseResultVerification_toGrpcResponseResultVerification() {
        ...
        GrpcResponseResultVerification grpcResponseResultVerification = grpcMapper.toGrpcResponseResultVerification(responseResultVerification);

        assertThat(responseResultVerification.getResultVerificationType().getCode()).isEqualTo(grpcResponseResultVerification.getResultVerificationType());
        assertThat(responseResultVerification.getApiResponseJsonType().getCode()).isEqualTo(grpcResponseResultVerification.getApiResponseJsonType());
        assertThat(responseResultVerification.getVerificationElementType().getCode()).isEqualTo(grpcResponseResultVerification.getVerificationElementType());
    }
    ...
    public void SceneCaseJobReport_toJobReport() {
        GrpcSceneCaseJobReport grpcSceneCaseJobReport = GrpcSceneCaseJobReport.newBuilder()
                .setJobId(ID).setMessage(MESSAGE).setErrCode(CODE).setDelayTimeTotalTimeCost(1).setDelayTimeTotalTimeCost(1).addCaseReportList(GrpcCaseReport.newBuilder().addInfoList(VALUE)).build();
        ...
        assertThat(grpcSceneCaseJobReport.getCaseReportListList().get(0).getInfoList(0)).isEqualTo(grpcSceneCaseJobReport.getCaseReportListList().get(0).getInfoList(0));
    }

    @Test
    public void ApiTestCaseJobReport_toJobReport() {
        GrpcCaseJobReport grpcCaseJobReport = GrpcCaseJobReport.newBuilder()
                .setJobId(ID).setMessage(MESSAGE).setErrCode(CODE).setCaseReport(GrpcCaseReport.newBuilder().setCaseId(ID).addInfoList(VALUE)).build();
        ...
        assertThat(grpcCaseJobReport.getCaseReport().getCaseId()).isEqualTo(apiTestCaseJobReport.getCaseReport().getCaseId());
        assertThat(grpcCaseJobReport.getCaseReport().getInfoList(0)).isEqualTo(apiTestCaseJobReport.getCaseReport().getInfoList().get(0));
    }

    @Test
    public void GrpcFunction_projectFunction() {
        ProjectFunctionEntity projectFunction = ProjectFunctionEntity.builder()
                .id(ID).projectId(ID).functionKey(KEY).functionName(NAME).functionCode(CODE).functionParams(List.of(ParamInfo.builder().key(KEY).build())).build();
        ...
        assertThat(projectFunction.getFunctionParams().get(0).getKey()).isEqualTo(grpcFunction.getFunctionParams(0).getKey());
    }

    @Test
    public void GrpcFunction_toGrpcFunction() {
        GlobalFunctionEntity globalFunction = GlobalFunctionEntity.builder().id(ID).workspaceId(ID).functionKey(KEY).functionName(NAME).functionCode(CODE).functionParams(List.of(ParamInfo.builder().key(KEY).build())).build();
        ...
        assertThat(globalFunction.getFunctionParams().get(0).getKey()).isEqualTo(grpcFunction.getFunctionParams(0).getKey());
    }
}
